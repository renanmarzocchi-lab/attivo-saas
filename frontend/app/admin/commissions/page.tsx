...
export default function AdminCommissions() {
  ...
  if (!res) return <div style={{ padding: 40, textAlign: 'center', color: '#6b7280' }}>Carregando comissões...</div>;

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: '#0B2442', margin: 0 }}>Comissões</h2>
        <p style={{ color: '#6b7280', fontSize: 14, marginTop: 4 }}>Visualize e gerencie as comissões dos afiliados</p>
      </div>
            ...
            <tbody>
              {res.data.map((c, i) => (
                <tr key={c.id} style={{ borderTop: '1px solid #f3f4f6', background: i % 2 === 0 ? '#fff' : '#fafbfc' }}>
                  <td style={{ ...TD, fontWeight: 500 }}>{c.affiliate.name}</td>
