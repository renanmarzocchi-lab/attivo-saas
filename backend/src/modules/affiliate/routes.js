...
export default async function affiliateDashboardRoutes(app) {
  ...
  // POST /affiliate/commissions/:id/request-withdrawal — solicita retirada de comissão disponível
  app.post('/affiliate/commissions/:id/request-withdrawal', {
    preHandler: [app.authenticate, app.authorize('AFFILIATE')],
  }, async (request, reply) => {
    ...
    if (commission.affiliateId !== affiliate.id) return reply.code(403).send({ message: 'Sem permissão' });
    if (commission.status !== 'AVAILABLE')       return reply.code(400).send({ message: 'Comissão não disponível para retirada' });
    if (commission.withdrawalRequest)            return reply.code(409).send({ message: 'Solicitação já existe' });
    ...
    const withdrawal = await prisma.withdrawalRequest.create({
      data: {
        ...
        pixKey:               pixKey ?? affiliate.pixKey ?? null,
        status:               'PENDING',
      },
    });
  ...
  // GET /affiliate/statement/summary — saldo consolidado
  app.get('/affiliate/statement/summary', {
    preHandler: [app.authenticate, app.authorize('AFFILIATE')],
  }, async (request, reply) => {
    const affiliate = await getAffiliate(request, reply);
    if (!affiliate) return;
    ...
    const [pending, available, paid] = await Promise.all([
      prisma.affiliateCommission.aggregate({ _sum: { amount: true }, where: { affiliateId: affiliate.id, status: 'PENDING' } }),
      prisma.affiliateCommission.aggregate({ _sum: { amount: true }, where: { affiliateId: affiliate.id, status: 'AVAILABLE' } }),
      prisma.affiliateCommission.aggregate({ _sum: { amount: true }, where: { affiliateId: affiliate.id, status: 'PAID' } }),
    ]);
