      ...
      <form onSubmit={handleSubmit}>
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            ...
            <div><label style={lbl}>Seguradora</label><input style={inp} value={form.insurer} onChange={e => set('insurer', e.target.value)} placeholder="Ex: Porto Seguro" /></div>
            <div><label style={lbl}>Nº Proposta</label><input style={inp} value={form.proposalNumber} onChange={e => set('proposalNumber', e.target.value)} /></div>
            <div><label style={lbl}>Nº Apólice</label><input style={inp} value={form.policyNumber} onChange={e => set('policyNumber', e.target.value)} /></div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
            <div><label style={lbl}>Nome *</label><input style={inp} value={form.customerName} onChange={e => set('customerName', e.target.value)} required placeholder="Nome completo" /></div>
            <div><label style={lbl}>CPF/CNPJ *</label><input style={inp} value={form.customerDocument} onChange={e => set('customerDocument', e.target.value)} required placeholder="000.000.000-00" /></div>
            <div><label style={lbl}>E-mail</label><input style={inp} type="email" value={form.customerEmail} onChange={e => set('customerEmail', e.target.value)} /></div>
            <div><label style={lbl}>Telefone</label><input style={inp} value={form.customerPhone} onChange={e => set('customerPhone', e.target.value)} placeholder="(00) 90000-0000" /></div>
            <div><label style={lbl}>Cidade/UF</label><input style={inp} value={form.customerCityUf} onChange={e => set('customerCityUf', e.target.value)} placeholder="São Paulo/SP" /></div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
            <div><label style={lbl}>Prêmio (R$)</label><input style={inp} type="number" step="0.01" min="0" value={form.premiumAmount} onChange={e => set('premiumAmount', e.target.value)} placeholder="0,00" /></div>
            <div><label style={lbl}>Comissão Est. (R$)</label><input style={inp} type="number" step="0.01" min="0" value={form.estimatedCommission} onChange={e => set('estimatedCommission', e.target.value)} placeholder="0,00" /></div>
            <div><label style={lbl}>Início da vigência</label><input style={inp} type="date" value={form.startDate} onChange={e => set('startDate', e.target.value)} /></div>
            <div><label style={lbl}>Fim da vigência</label><input style={inp} type="date" value={form.endDate} onChange={e => set('endDate', e.target.value)} /></div>
            <div><label style={lbl}>Alerta renovação (dias antes)</label><input style={inp} type="number" min="1" max="365" value={form.renewalReminderDays} onChange={e => set('renewalReminderDays', e.target.value)} /></div>
          </div>
        </div>

        <div className="card" style={{ marginBottom: 16 }}>
          <h4 style={{ marginBottom: 12, color: '#374151' }}>Observações</h4>
          <textarea style={{ ...inp, height: 80, resize: 'vertical' }} value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Notas internas..." />
        </div>
